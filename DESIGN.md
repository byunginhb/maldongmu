...
도트 감성은 **제목 타이포(도트 폰트)**와 **소소한 장식(구분선·포인트 아이콘)**에만 쓰고, 아바타는 실제 사진형을 사용한다.
...
왼쪽에 사진형 아바타(48~56px, radius 12px), 오른쪽에 이름+나이(15px/600) → 직업·지역(13px, brown-soft)
...
왼쪽 정렬 + 옆에 사진형 아바타 28px. radius 18px, 말꼬리 없음. 버블 최대 폭 78%.
...
- 아바타는 항상 성별·연령대별 WebP 사진 세트 사용 (uuid가 같으면 같은 얼굴)
...
카드는 paper+line 테두리 radius 16px, 그림자 없음, 사진형 아바타 필수" — 이 한 줄이 지켜지면 말동무다.
