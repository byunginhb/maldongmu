...
export function interviewMessages(detail: any, topic: string, questions: string[]) {
  ...
  return [
    ...
    {
      role: "user" as const,
      content: `지금부터 '이웃 인터뷰'에 참여합니다. "${topic}"에 대한 설문이에요. 아래 질문에 당신으로서, 당신의 삶·경험·형편에 비추어 1인칭으로 솔직하게 답해주세요. 질문마다 "Q. 질문" 을 그대로 적고 다음 줄에 답을 쓰세요. 표정·몸짓은 위 규칙대로 가끔만 괄호로 섞고, 강의하듯 길게 늘어놓지 말고 대화체로 답하세요.
