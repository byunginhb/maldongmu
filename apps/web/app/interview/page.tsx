...
/** 게스트에게 보여줄 고정 샘플 (LLM 호출·크레딧 0) */
function GuestTeaser({ onLogin }: { onLogin: () => void }) {
  return (
    <>
      ...
      <p className="meta" style={{ margin: "18px 0 8px" }}>그리고 이런 리포트가 나와요</p>
      <div className="iv-lock">
        <section className="iv-report">
          <h3>핵심 인사이트</h3>
          <p>시니어층은 “본인”보다 <strong>“손주 매개” 구매</strong>가 우세했어요. 가격보다 건강 이미지에 반응합니다.</p>
          <div className="iv-quote">“손주가 사달라믄 사주긴 할 겨” — 김정순, 68</div>
          <h3>이렇게 해보면 어때요</h3>
          <p>시니어 타깃은 ‘손주 선물’ 메시지로, 가격은…</p>
        </section>
        <div className="iv-lock-fade"><span>로그인하면 이웃 5명의 답변과 전체 리포트가 열려요</span></div>
      </div>

      <div style={{ marginTop: 20 }}>
        <button className="btn-cta" onClick={onLogin}>로그인하고 내 주제로 인터뷰하기</button>
        <p className="meta" style={{ textAlign: "center", margin: "10px 0 0" }}>가입하면 2번 무료로 해볼 수 있어요</p>
      </div>
    </>
  );
}
  ...
  return (
    <main className="page">
      ...
      {me === null ? (
        ...
        <GuestTeaser onLogin={() => setShowLogin(true)} />
